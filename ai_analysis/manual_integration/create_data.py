...
def create_anagrafica(sheet,row_supply):
    ...
    for index_cells in index_row:
        if(index_cells.value):
            to_return[index_cells.value]=[sheet[uc.get_column_letter(index_cells.column)+str(index_cells.row+1)].value]
...
def create_forecast(sheet,row_supply):
    ...
    for index_cells in index_row:
        if(index_cells.value and('2' in index_cells.value or 'Material'==index_cells.value)):
            to_return[index_cells.value]=[sheet[uc.get_column_letter(index_cells.column)+str(index_cells.row+1)].value,sheet[index_cells.column+str(index_cells.row+2)].value]
...
def create_sales_data(sheet,row_begin,row_end):
    ...
    for index_cells in index_row:
        if(index_cells.value):
            to_return[index_cells.value]=[sheet[uc.get_column_letter(index_cells.column)+str(x)].value or 0 for x in range(row_begin+2,row_end)]
...
def create_market_data(sheet,row_begin,row_end):
    ...
    for index_cells in index_row:
        if(index_cells.value):
            to_return[str(index_cells.value).replace('Sell-in UN Month','01')]=[sheet[uc.get_column_letter(index_cells.column)+str(x)].value for x in range(row_begin+2,row_end)]
    return pd.DataFrame.from_records(to_return)
